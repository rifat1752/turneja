    ...
    disableOnInteraction: false, // Keep autoplay even after user interacts
                ...
                {bannerImage.map((banner, index) => (
                    <SwiperSlide key={index}>
                       ...
                       <div className='absolute   flex flex-col  items-start top-5 sm:top-0 left-0 pl-16 sm:w-5/6 md:w-3/5 xl:w-1/2  h-full  xl:h-3/4 justify-center    bg-gradient-to-r from-purple-600 to-transparent  '>
                       <h1 className='text-slate-200 font-bold text-xl sm:text-4xl lg:text-5xl mt-5'>{banner.heading}</h1>
                       <p className='text-slate-300 text-xs sm:text-base lg:text-xl  text-left mt-2 ms:mt-10 mr-10'>{banner.Description}</p>
                       <Link to='/signup' className='banner-btn text-sm md:text-lg font-semibold w-28 sm:w-40 h-11  mt-5 py-3 sm:py-2'>Sign Up Now!</Link>
                       </div>
